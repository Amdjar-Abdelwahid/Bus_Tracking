        ...
        busStopUseCases.getAllBusStops().onEach { result->
            uiState = when(result){
                is Resource.Success ->{
                    ...
                    val userLon = uiState.location!!.longitude
                    val sortedStops = result.data?.sortedBy { stop ->
                        com.example.bustrackingapp.core.util.LocationUtil.haversine(
                            userLat, userLon, stop.location.lat, stop.location.lng
                        )
                    } ?: emptyList()
                    uiState.copy(
                        nearbyBusStops = sortedStops.take(5), // Les 5 plus proches
                        isLoadingNearbyStops = false,
                        isRefreshingNearbyStops = false,
                        errorNearbyBuses = null,
                    )
